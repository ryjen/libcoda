...
namespace arg3
{
	...
	public:
		// constructors
		variant_value() : mValue() {}
		variant_value(const value_type &value) : mValue(value) {}
		variant_value(const T *value) : mValue(value) {}
		variant_value(T value) : mValue(1, value) {}
		...
		int base_int(int base) const {
			return std::stoi(mValue, NULL, base);
		}
		...
		unsigned base_uint(int base) const {
			...
			return static_cast<unsigned>(std::stoul(mValue, NULL, base));
		}
		...
		long base_long(int base) const {
			return std::stol(mValue, NULL, base);
		}
		...
		unsigned long base_ulong(int base) const {
			return std::stoul(mValue, NULL, base);
		}
		...
		long long base_llong(int base) const {
			return std::stoll(mValue, NULL, base);
		}
		...
		unsigned long long base_ullong(int base) const {
			return std::stoull(mValue, NULL, base);
		}
		...
		// boolean
		bool to_bool() const {
			return mValue != "false" && to_int(0) != 0;
		}
		...
		variant_value<T> &operator <<(const V &value) {
			mValue.append(std::to_string(value));
			return *this;
		}
	...
	template<typename T>
	inline ostream& operator<<(ostream& stream, const variant_value<T>& v) {
		stream << v.to_cstring();
		return stream;
 	}
 	...
 	template<typename T>
 	inline bool operator==(const typename variant_value<T>::value_type &lhs, const variant_value<T> &v) {
 		return lhs == v.to_string();
 	}

 	template<typename T>
 	inline bool operator==(const T* lhs, const variant_value<T> &v) {
 		return variant_value<T>(lhs) == v;
 	}

 	template<typename T>
 	inline bool operator==(T lhs, const variant_value<T> &v) {
 		return lhs == v.to_char();
 	}

 	template<typename T>
 	inline bool operator==(long lhs, const variant_value<T> &v) {
 		return lhs == v.to_long();
 	}
 	
 	template<typename T>
 	inline bool operator==(unsigned long lhs, const variant_value<T> &v) {
 		return lhs == v.to_ulong();
 	}

 	template<typename T>
 	inline bool operator==(int lhs, const variant_value<T> &v) {
 		return lhs == v.to_int();
 	}

 	template<typename T>
 	inline bool operator==(unsigned lhs, const variant_value<T> &v) {
 		return lhs == v.to_uint();
 	}

 	template<typename T>
 	inline bool operator==(long long lhs, const variant_value<T> &v) {
 		return lhs == v.to_llong();
 	}

 	template<typename T>
 	inline bool operator==(unsigned long long lhs, const variant_value<T> &v) {
 		return lhs == v.to_ullong();
 	}

 	template<typename T>
 	inline bool operator==(float lhs, const variant_value<T> &v) {
 		return lhs == v.to_float();
 	}

 	template<typename T>
 	inline bool operator==(double lhs, const variant_value<T> &v) {
 		return lhs == v.to_double();
 	}
 	
 	template<typename T>
 	inline bool operator==(long double lhs, const variant_value<T> &v) {
 		return lhs == v.to_ldouble();
 	}

 	template<typename T>
 	inline bool operator==(bool lhs, const variant_value<T> &v) {
