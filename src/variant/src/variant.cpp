...
namespace coda {
  const char *const variant::TYPE_NAMES[] = {
      "nulltype", "number",  "unumber", "real",  "binary",
      "string",   "wstring", "char",    "wchar", "bool"};

  variant::variant() : type_(NULLTYPE), num_(0) {}

  variant::variant(const nullptr_t &null)
      : type_(NULLTYPE), num_(0), size_(0) {}

  variant::variant(bool value)
      : type_(BOOL), num_(value), size_(sizeof(bool)) {}

  variant::variant(char c) : type_(CHAR), num_(c), size_(sizeof(char)) {}

  variant::variant(unsigned char uc)
      : type_(CHAR), unum_(uc), size_(sizeof(unsigned char)) {}

  variant::variant(wchar_t wc)
      : type_(WCHAR), num_(wc), size_(sizeof(wchar_t)) {}

  variant::variant(short s) : type_(NUMBER), num_(s), size_(sizeof(short)) {}

  variant::variant(unsigned short us)
      : type_(UNUMBER), unum_(us), size_(sizeof(unsigned short)) {}

  variant::variant(int i) : type_(NUMBER), num_(i), size_(sizeof(int)) {}

  variant::variant(unsigned ui)
      : type_(UNUMBER), unum_(ui), size_(sizeof(unsigned)) {}

  variant::variant(long l) : type_(NUMBER), num_(l), size_(sizeof(long)) {}

  variant::variant(unsigned long ul)
      : type_(UNUMBER), unum_(ul), size_(sizeof(unsigned long)) {}
  ...
  variant::variant(long double ld)
      : type_(REAL), real_(ld), size_(sizeof(long double)) {}

  variant::variant(long long ll)
      : type_(NUMBER), num_(ll), size_(sizeof(long long)) {}

  variant::variant(unsigned long long ull)
      : type_(UNUMBER), unum_(ull), size_(sizeof(unsigned long long)) {}

  variant::variant(const char *str)
      : type_(STRING), str_(str == nullptr ? "" : str),
        size_(str == nullptr ? 0 : strlen(str)) {}

  variant::variant(const wchar_t *str)
      : type_(WSTRING), wstr_(str == nullptr ? L"" : str),
        size_(str == nullptr ? 0 : wcslen(str)) {}

  variant::variant(const string &str)
      : type_(STRING), str_(str), size_(str.size()) {}

  variant::variant(const wstring &str)
      : type_(WSTRING), wstr_(str), size_(str.size()) {}

  variant::variant(const binary &bin)
      : type_(BINARY), ptr_(bin), size_(bin.size()) {}

  variant::variant(const std::shared_ptr<complex> &c)
      : type_(COMPLEX), compl_(c), size_(c->size()) {}
  ...
  size_t variant::size() const { return size_; }

  variant::types variant::type() const { return type_; }

  std::string variant::type_name() const { return TYPE_NAMES[type_]; }

  // operators
  variant::operator string() const { return to_string(); }

  variant::operator int() const { return to_int(); }

  variant::operator long() const { return to_long(); }

  variant::operator long long() const { return to_llong(); }

  variant::operator long double() const { return to_ldouble(); }

  variant::operator unsigned long() const { return to_ulong(); }
  variant::operator unsigned long long() const { return to_ullong(); }

  variant::operator double() const { return to_double(); }

  variant::operator float() const { return to_float(); }

  variant::operator bool() const { return to_bool(); }

  variant::operator wstring() const { return to_wstring(); }

  variant::operator unsigned() const { return to_uint(); }

  variant::operator const wchar_t *() const { return to_wcstring(); }

  variant::operator const char *() const { return to_cstring(); }

  variant::operator char() const { return to_char(); }

  variant::operator binary() const { return to_binary(); }

  bool variant::operator==(const nullptr_t &other) const { return is_null(); }

  bool variant::operator==(bool other) const { return to_bool() == other; }

  bool variant::operator==(float other) const { return to_float() == other; }

  bool variant::operator==(double other) const { return to_double() == other; }
  ...
  bool variant::operator==(long long ll) const { return to_llong() == ll; }

  bool variant::operator==(long l) const { return to_long() == l; }

  bool variant::operator==(int i) const { return to_int() == i; }

  bool variant::operator==(unsigned ui) const { return to_uint() == ui; }

  bool variant::operator==(unsigned long ul) const { return to_ulong() == ul; }

  bool variant::operator==(const char *str) const { return to_string() == str; }
  ...
  bool variant::operator!=(bool other) const { return !operator==(other); }
  bool variant::operator!=(float other) const { return !operator==(other); }
  bool variant::operator!=(double other) const { return !operator==(other); }
  bool variant::operator!=(long double other) const {
  ...
  bool variant::operator!=(long long other) const { return !operator==(other); }
  bool variant::operator!=(long other) const { return !operator==(other); }
  bool variant::operator!=(int other) const { return !operator==(other); }
  bool variant::operator!=(unsigned other) const { return !operator==(other); }
  ...
  bool variant::operator==(const variant &value) const { return equals(value); }
  ...
  bool variant::is_complex() const { return type_ == COMPLEX && compl_; }
  ...
  unsigned long long variant::to_ullong() const {
    return convert_base<unsigned long long, unsigned long long>(stoull, stoull,
                                                                BASE);
  }
  ...
  float variant::to_float() const { return convert<float, float>(stof, stof); }
  ...
  bool operator==(const nullptr_t &f, const variant &v) { return v.is_null(); }

  bool operator==(float f, const variant &v) { return f == v.to_float(); }
  ...
  bool operator==(long long ll, const variant &v) { return ll == v.to_llong(); }

  bool operator==(long l, const variant &v) { return l == v.to_long(); }

  bool operator==(int i, const variant &v) { return i == v.to_int(); }

  bool operator==(unsigned ui, const variant &v) { return ui == v.to_uint(); }

  bool operator==(unsigned long ul, const variant &v) {
    return ul == v.to_ulong();
  }

  bool operator==(bool b, const variant &v) { return b == v.to_bool(); }

  bool operator==(double d, const variant &v) { return d == v.to_double(); }

  bool operator==(long double d, const variant &v) {
    return d == v.to_ldouble();
  }

  bool operator==(const binary &bin, const variant &v) { return v == bin; }
  ...
  bool operator!=(float f, const variant &v) { return !operator==(f, v); }
  ...
  bool operator!=(long long ll, const variant &v) { return !operator==(ll, v); }
  bool operator!=(long l, const variant &v) { return !operator==(l, v); }
  bool operator!=(int i, const variant &v) { return !operator==(i, v); }
  bool operator!=(unsigned ui, const variant &v) { return !operator==(ui, v); }
  bool operator!=(unsigned long ul, const variant &v) {
    return !operator==(ul, v);
  }
  bool operator!=(bool b, const variant &v) { return !operator==(b, v); }
  bool operator!=(double d, const variant &v) { return !operator==(d, v); }
  ...
  // Custom type
  int variant::complex::to_int() const { throw illegal_conversion(); }
  unsigned variant::complex::to_uint() const { throw illegal_conversion(); }
  long variant::complex::to_long() const { throw illegal_conversion(); }
  unsigned long variant::complex::to_ulong() const {
    throw illegal_conversion();
  }
  long long variant::complex::to_llong() const { throw illegal_conversion(); }
  unsigned long long variant::complex::to_ullong() const {
    throw illegal_conversion();
  }
  double variant::complex::to_double() const { throw illegal_conversion(); }
  long double variant::complex::to_ldouble() const {
    throw illegal_conversion();
  }
  bool variant::complex::to_bool() const { throw illegal_conversion(); }
  float variant::complex::to_float() const { throw illegal_conversion(); }
  const void *variant::complex::to_pointer() const {
    throw illegal_conversion();
  }
  binary variant::complex::to_binary() const { throw illegal_conversion(); }
  char variant::complex::to_char() const { throw illegal_conversion(); }
  unsigned char variant::complex::to_uchar() const {
    throw illegal_conversion();
  }
  wchar_t variant::complex::to_wchar() const { throw illegal_conversion(); }
  string variant::complex::to_string() const { throw illegal_conversion(); }
  ...
  wstring variant::complex::to_wstring() const { throw illegal_conversion(); }

  variant::complex::operator string() const { return to_string(); }
  variant::complex::operator int() const { return to_int(); }
  variant::complex::operator long() const { return to_long(); }
  variant::complex::operator long long() const { return to_llong(); }
  variant::complex::operator unsigned long() const { return to_ulong(); }
  variant::complex::operator unsigned long long() const { return to_ullong(); }
  variant::complex::operator double() const { return to_double(); }
  variant::complex::operator long double() const { return to_ldouble(); }
  variant::complex::operator float() const { return to_float(); }
  variant::complex::operator bool() const { return to_bool(); }
  variant::complex::operator wstring() const { return to_wstring(); }
  variant::complex::operator unsigned() const { return to_uint(); }
  variant::complex::operator char() const { return to_char(); }
  variant::complex::operator unsigned char() const { return to_uchar(); }
  variant::complex::operator wchar_t() const { return to_wchar(); }
  variant::complex::operator const wchar_t *() const { return to_wcstring(); }
  variant::complex::operator const char *() const { return to_cstring(); }
  variant::complex::operator binary() const { return to_binary(); }
  ...
  string to_string(const variant::complex &value) { return value.to_string(); }
} // namespace coda
