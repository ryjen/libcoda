...
namespace coda {
  ...
  class illegal_conversion : std::runtime_error {
     ...
     */
    explicit illegal_conversion(const std::string &what_arg)
        : std::runtime_error(what_arg) {}

    /*!
     * default constructor
     */
    explicit illegal_conversion()
        : std::runtime_error("no conversion available") {}
  };
  ...
  class variant {
     ...
     */
    template <typename T> T convert_numeric() const {
      switch (type_) {
      case BOOL:
      case CHAR:
      case WCHAR:
      case NUMBER: {
        ...
        if (std::is_unsigned<T>::value) {
          ...
          if (static_cast<T>(num_) <= std::numeric_limits<T>::max() &&
              static_cast<T>(num_) >= std::numeric_limits<T>::min()) {
            return static_cast<T>(num_);
          }
        }
        /* test signed ranges and cast the value */
        else if (num_ <= std::numeric_limits<
                             typename try_make_signed<T>::type>::max() &&
                 num_ >= std::numeric_limits<
                             typename try_make_signed<T>::type>::min()) {
          return static_cast<T>(num_);
        }
        break;
      }
      case UNUMBER: {
        ...
        } else if (unum_ <= std::numeric_limits<
                                typename try_make_unsigned<T>::type>::max() &&
                   unum_ >= std::numeric_limits<
                                typename try_make_unsigned<T>::type>::min()) {
          return static_cast<T>(unum_);
        }
        break;
      }
      case REAL:
        if (real_ <= std::numeric_limits<T>::max() &&
            real_ >= std::numeric_limits<T>::lowest()) {
     ...
     * a string function
     */
    template <class T> using str_fn = T (*)(const std::string &, size_t *);

    /*!
     * a wide string function
     */
    template <class T> using wstr_fn = T (*)(const std::wstring &, size_t *);

    /*!
     * attempts to convert to a numeric using a base
     */
    template <typename T, typename R>
    T convert_base(str_base_fn<R> strfunk, wstr_base_fn<R> wstrfunk,
                   int base) const {
