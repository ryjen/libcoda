...
namespace coda {
  namespace json {
    array_iterator::array_iterator(const array_iterator &other)
        : value_(other.value_), pos_(other.pos_),
          ref_(json_object_array_get_idx(value_, pos_)) {}

    array_iterator::array_iterator(json_object *value, long pos)
        : value_(value), pos_(pos),
          ref_(json_object_array_get_idx(value_, pos_)) {}

    array_iterator::array_iterator(array_iterator &&other)
        : value_(other.value_), pos_(other.pos_), ref_(other.ref_) {
      other.value_ = NULL;
      other.ref_ = 0;
      other.pos_ = 0;
    }
    ...
    array_iterator::reference array_iterator::operator*() { return ref_; }

    array_iterator::value_type array_iterator::operator*() const {
      return ref_;
    }

    array_iterator::pointer array_iterator::operator->() { return &ref_; }

    array_iterator::value_type array_iterator::
    operator[](const difference_type &idx) const {
    ...
    bool array_iterator::operator==(const array_iterator &other) const {
      return pos_ == other.pos_ && json_object_array_length(value_) ==
                                       json_object_array_length(other.value_);
    }
