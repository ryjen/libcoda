...
namespace coda {
  namespace json {
    ...
    object::object(const string &value)
        : value_(json_object_new_string(value.c_str())) {}
    ...
    int object::get_int(const std::string &key) const {
      json_object *obj;
#ifdef JSON_C_EXTENDED
      if (!json_object_object_get_ex(value_, key.c_str(), &obj))
    ...
    bool object::is_valid() const { return value_ != NULL; }
    void object::foreach (
        std::function<void(const char *key, const json_object *value)> funk)
        const {
      for (struct lh_entry *entry = json_object_get_object(value_)->head; entry;
           entry = entry->next) {
    ...
    void object::merge(const object &other) {
      for (struct lh_entry *entry = json_object_get_object(other.value_)->head;
    ...
    size_t object::size() const {
      if (is_object()) {
        ...
        foreach (
            [&iCount](const char *key, const json_object *value) { iCount++; })
          ;
    ...
    void object::set_null(const std::string &key) { set_value(key, NULL); }
    ...
    object_iterator object::end() { return object_iterator(NULL); }
    object_iterator object::begin() const {
      return object_iterator(json_object_get_object(value_)->head);
    }
    object_iterator object::end() const { return object_iterator(NULL); }
    int object::to_int() const { return json_object_get_int(value_); }
    ...
    bool object::to_bool() const { return json_object_get_boolean(value_); }
    double object::to_double() const { return json_object_get_double(value_); }
    ...
    object::operator int() const { return to_int(); }
    object::operator long long() const { return to_int64(); }
    object::operator bool() const { return to_bool(); }
    object::operator double() const { return to_double(); }
    ...
    object::operator string() const { return to_string(); }
