...
namespace coda {
  bool nullstr(const char *str) { return !str || !*str; }
  ...
  bool is_valid_email(const string &address) {
    ...
    if (c == address.begin() || *(c - 1) == '.')
      return 0;

    /* next we validate the domain portion (name@domain) */
    if ((domain = ++c) == address.cend())
      return 0;
  ...
  string join(const string &value, string::size_type count,
  ...
  string join(string::value_type value, string::size_type count,
              const string &delimiter) {
    ostringstream buf;
  ...
  bool prefix(const string &astr, const string &bstr, bool caseSensitive) {
    ...
    for (auto a = astr.cbegin(), b = bstr.cbegin(); a != astr.cend();
         a++, b++) {
      if (caseSensitive ? (*a != *b) : (tolower(*a) != tolower(*b)))
        return false;
    }
  ...
  bool suffix(const string &astr, const string &bstr, bool caseSensitive) {
    ...
    if (sstr1 <= sstr2 &&
        equals(astr, bstr.substr(sstr2 - sstr1), caseSensitive))
      return true;
    else
      return false;
  }
  ...
  bool contains(const string &astr, const string &bstr, bool caseSensitive) {
    ...
    for (ichar = 0; ichar <= sstr2 - sstr1; ichar++) {
      if (c0 == (caseSensitive ? bstr[ichar] : tolower(bstr[ichar])) &&
          prefix(astr, bstr.substr(ichar), caseSensitive))
        return true;
    }
  ...
  std::string &ltrim(std::string &s) {
    s.erase(s.begin(),
            std::find_if(s.begin(), s.end(), [](std::string::value_type c) {
