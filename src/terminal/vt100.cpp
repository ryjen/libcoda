...
namespace coda {
  namespace terminal {
    ...
    shared_ptr<csi_code>
    vt100::parse_csi_code(data_buffer &output,
                          data_buffer::const_iterator &start,
                          data_buffer::const_iterator end) const {
    ...
    void csi_code::add_value(int value) { values_.push_back(value); }

    void csi_code::set_prefix(char prefix) noexcept { prefix_ = prefix; }

    void csi_code::set_command(char command) noexcept { command_ = command; }
